...
//   }
//
//   override create<T>(name: string, initialValue: T): BaseField<T> {
//     return this.add(new BaseField<T>(name, initialValue));
//   }
//
//   override upset<T>(name: string, value: T): BaseField<T> {
//     if (this.has(name)) {
...
//   }
//
//   override get<T>(name: string): BaseField<T> {
//     throwIf(!this._fields.has(name), `Field with name '${name}' not exists`);
//     return this._fields.get(name) as BaseField<T>;
//   }
// }
