...
[@axi-engine/data](../README.md) / CoreNumericField
...
> `protected` `readonly` **\_onChange**: `Emitter`\<\{ `newValue`: `number`; `oldValue`: `number`; \}\>
...
> `readonly` **onChange**: `Subscribable`\<\{ `newValue`: `number`; `oldValue`: `number`; \}\>
