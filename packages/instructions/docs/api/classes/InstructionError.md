...
[@axi-engine/instructions](../README.md) / InstructionError

# Class: InstructionError

Defined in: packages/instructions/src/errors.ts:7
...
> **new InstructionError**(`message`, `instruction`, `options?`): `InstructionError`

Defined in: packages/instructions/src/errors.ts:16
...
The instruction object associated with the failure.

##### options?

`ErrorOptions`

Standard error options (e.g., for chaining causes).
...
> `readonly` **instruction**: [`Instruction`](../type-aliases/Instruction.md)

Defined in: packages/instructions/src/errors.ts:9
